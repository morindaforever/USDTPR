...
export function AdminSettingsPage() {
  ...
  return (
    <div>
      ...
      {query.isLoading ? (
        <div className="space-y-2">{[0, 1, 2, 3].map((i) => <div key={i} className="h-12 animate-pulse rounded-xl bg-surface-200/40" />)}</div>
      ) : query.error ? (
        <div className="rounded-2xl border border-surface-200 bg-surface-50 p-6 text-center text-sm text-surface-300">{query.error}</div>
      ) : settings.length === 0 ? (
        <div className="rounded-2xl border border-dashed border-surface-200 p-8 text-center text-sm text-surface-400">No settings in this group</div>
      ) : (
          ...
          <table className="w-full text-left text-sm">
            ...
            <tbody className="divide-y divide-white/5">
              {settings.map((setting) => (
                <tr key={setting.key} className="text-surface-200 transition-colors hover:bg-surface-200/40">
                  <td className="px-4 py-3">
                    <span className="font-mono text-xs text-surface-100">{setting.key}</span>
                    <span className="ml-2 rounded bg-surface-200/40 px-1.5 py-0.5 text-[10px] uppercase text-surface-500">{setting.value_type}</span>
                  </td>
