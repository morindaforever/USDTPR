...
export function ErrorState({
  ...
  return (
    <div
      role="alert"
      className="flex flex-col items-center justify-center gap-2 rounded-2xl border border-danger-500/20 bg-danger-500/5 px-6 py-10 text-center"
    >
      <div className="flex h-11 w-11 items-center justify-center rounded-full bg-danger-500/15 text-danger-500">
        <AlertTriangle className="h-5 w-5" aria-hidden />
      </div>
      <p className="text-sm font-semibold text-danger-600">{title}</p>
      <p className="max-w-xs text-xs leading-relaxed text-surface-500">{message}</p>
      {onRetry && (
